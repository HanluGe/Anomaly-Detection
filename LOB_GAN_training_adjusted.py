# -*- coding: utf-8 -*-
"""
Adjusted version for Q1+Q2:
- Data handling, training loop, early stopping: identical to LOB_GAN_training_raw.py
- Only change: Generator forward uses lay8 as a small residual head.
"""

import os

# 防止 Windows 下 OpenMP 冲突导致 kernel 直接崩溃
...
def train_lob_gan(
    ...
    tradingDays = [
        ...
        "2024-01-11","2024-01-12","2024-01-15","2024-01-16","2024-01-17",
        "2024-01-18","2024-01-19","2024-01-22","2024-01-23","2024-01-24",
        "2024-01-25","2024-01-26","2024-01-29","2024-01-30","2024-01-31",
        "2024-02-01","2024-02-02","2024-02-15","2024-02-16","2024-02-19",
        "2024-02-20","2024-02-21","2024-02-22","2024-02-23","2024-02-26",
        "2024-02-27","2024-02-29","2024-03-01","2024-03-04","2024-03-05",
        ...
        "2024-07-23","2024-07-26","2024-07-29","2024-07-30","2024-07-31",
        ...
        "2024-09-12","2024-09-13","2024-09-16","2024-09-18","2024-09-19",
        "2024-09-20","2024-09-23","2024-09-24","2024-09-25","2024-09-26",
        "2024-09-27","2024-09-30","2024-10-01","2024-10-02","2024-10-03",
        "2024-10-04","2024-10-07","2024-10-08","2024-10-09","2024-10-11",
    ...
    file1Path = os.path.join(DATA_DIR, stock + '_md_202310_202310.csv.gz')
    file2Path = os.path.join(DATA_DIR, stock + '_md_202311_202311.csv.gz')
    file3Path = os.path.join(DATA_DIR, stock + '_md_202312_202312.csv.gz')
    ...
    for epoch in range(epochs):
        for i, data in enumerate(train_dataloader):
            ...
            if i % 10 == 0:
                print("[Epoch %d/%d][Batch %d/%d][D train loss: %f][G train loss: %f]" %
                      (epoch + 1, epochs, i + 1, len(train_dataloader),
        ...
        print("[Epoch %d/%d][Batch %d/%d][D eval loss: %f][G eval loss: %f]" %
              (epoch + 1, epochs, i + 1, len(eval_dataloader),
               d_loss_total.item() / len(eval_dataloader),
               g_loss_total.item() / len(eval_dataloader)))

        train_verge.append(get_verge(
            train_g_loss[-len(train_dataloader):],
            train_d_loss[-len(train_dataloader):]
        ))
        eval_verge.append(get_verge(
            eval_g_loss[-len(eval_dataloader):],
            eval_d_loss[-len(eval_dataloader):]
        ))

        if epoch >= 5:
            if train_verge[-3] > train_verge[-2] and train_verge[-2] > train_verge[-1] and \
               eval_verge[-3] < eval_verge[-2] and eval_verge[-2] < eval_verge[-1]:
                print("Early stopping.")
                break

    train_csv_path = os.path.join(RESULTS_DIR, stock + '_train_g_d_adjusted.csv')
    eval_csv_path = os.path.join(RESULTS_DIR, stock + '_eval_g_d_adjusted.csv')
    pd.DataFrame([train_g_loss, train_d_loss], index=['train_g', 'train_d']).to_csv(train_csv_path)
    pd.DataFrame([eval_g_loss, eval_d_loss], index=['eval_g', 'eval_d']).to_csv(eval_csv_path)

    try:
        png_path = os.path.join(RESULTS_DIR, f"{stock}_loss_curve_adjusted.png")
        plt.figure(figsize=(10, 6))
        plt.plot(train_g_loss, label='Train G loss')
        plt.plot(eval_g_loss, label='Eval G loss')
        plt.plot(train_d_loss, label='Train D loss')
        plt.plot(eval_d_loss, label='Eval D loss')
        plt.xlabel('Iteration')
        plt.ylabel('Loss')
        plt.title(f'GAN Training Loss (adjusted G) - {stock}')
        plt.legend()
        plt.tight_layout()
        ...
        print("Warning: failed to save loss curve:", repr(e))
    try:
        gen_path = os.path.join(RESULTS_DIR, stock + '_generator1_adjusted.pth')
        disc_path = os.path.join(RESULTS_DIR, stock + '_discriminator1_adjusted.pth')
        torch.save(generator, gen_path)
        torch.save(discriminator, disc_path)
        print("Saved models to", gen_path, "and", disc_path)
    except Exception as e:
        print("Warning: failed to save models:", repr(e))

    print("Done training ADJUSTED LOB_GAN for stock " + stock)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--lr_g", type=float, default=0.00375)
